...
class PETRHead(AnchorFreeHead):
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        for lvl in range(hs.shape[0]):
            ...
            # if lvl == 0 and torch.rand(1).item() < 0.001:
            #     print("reference min/max:", float(reference.min()), float(reference.max()))
            #     print("tmp_kpt(before add) min/max:", float(tmp_kpt.min()), float(tmp_kpt.max()))
            ...
            reference = inverse_sigmoid(reference)  # 把 [0,1] 变成 logit
            tmp_kpt = tmp_kpt + reference
            outputs_kpt = tmp_kpt.sigmoid()
    ...
    def _pool_memory(self, memory, bs):
        """Pool transformer memory into one token vector per sample."""
        if memory is None:
            raise ValueError('memory is None, cannot pool token')

        if memory.dim() == 4:
            return memory.mean(dim=(2, 3))
        ...
        if memory.dim() == 3:
            if memory.size(0) == bs:
                ...
                raise RuntimeError(
                    f'Unexpected memory batch dimension: {tuple(memory.shape)}')
    ...
    def forward_train(self,
        ...
        if self.with_token_distill and self.distill_stage in (1, 2):
            ...
            if gt_token is None:
                raise ValueError(
                    'gt_token is required for distill_stage=1/2, but got None')

            if isinstance(gt_token, (list, tuple)):
                gt_token = torch.stack(
                    [t.to(memory.device).float() for t in gt_token], dim=0)
            else:
                gt_token = gt_token.to(memory.device).float()
                if gt_token.dim() == 1:
                    gt_token = gt_token.unsqueeze(0)

            if gt_token.dim() != 2 or gt_token.size(1) != self.token_dim:
                raise ValueError(
                    f'gt_token shape should be (B,{self.token_dim}), '
                    f'but got {tuple(gt_token.shape)}')

            pooled = self._pool_memory(memory, bs)
            pred_token = self.token_head(pooled)
    ...
    def loss_single(self,
        ...
        #     W = kpt_weights.reshape(-1, 14, 2)
        #     print("kpt_weights mean x/y:", float(W[..., 0].mean()), float(W[..., 1].mean()))
        ...
        # if torch.rand(1).item() < 0.001:
        #     #P = kpt_preds.reshape(-1, 14, 2)
        #     T = kpt_targets.reshape(-1, 14, 2)
        #     W = kpt_weights.reshape(-1, 14, 2)
        #     print("T x(min/max/mean):", float(T[..., 0].min()), float(T[..., 0].max()), float(T[..., 0].mean()))
        #     print("T y(min/max/mean):", float(T[..., 1].min()), float(T[..., 1].max()), float(T[..., 1].mean()))
        #     print("W x/y mean:", float(W[..., 0].mean()), float(W[..., 1].mean()))
        ...
        weight_enhance = torch.ones_like(kpt_preds).reshape(-1, 14, 2)
        #weight_enhance[:,enp,:] = weight_enhance[:,enp,:]*3
        weight_enhance[:, enp, :] = weight_enhance[:, enp, :] * 2


        #weight_enhance[:, :, 2] = 0.0

        #weight_enhance = weight_enhance.reshape(-1,42)
        weight_enhance = weight_enhance.reshape(-1, 28)
    ...
    def _get_target_single(self,
        ...
        # keypoint targets
        # keypoint targets (2D)
        kpt_targets = torch.zeros_like(kpt_pred)  # (num_query, K*2) e.g. (100, 28)
        kpt_weights = torch.zeros_like(kpt_pred)  # same shape

        pos_gt_kpts = gt_keypoints[sampling_result.pos_assigned_gt_inds]  # (num_pos, K*3) flattened

        # ---- 打印 1：正样本数量（建议 5% 概率）----
        # if torch.rand(1).item() < 0.05:
        #     print("[_get_target_single] num_pos:", int(pos_inds.numel()),
        #           "num_gt:", int(gt_keypoints.size(0)),
        #           "kpt_pred shape:", tuple(kpt_pred.shape),
        #           "pos_gt_kpts shape:", tuple(pos_gt_kpts.shape))

        num_pos = pos_inds.numel()
        if num_pos > 0:
            # 关键：先 reshape 成 (num_pos, K, 3)，再取 xy
            # 注意：这里 K 用 self.num_keypoints（你的 config 里是 14）
            pos_gt_kpts_reshaped = pos_gt_kpts.reshape(num_pos, self.num_keypoints, 3)
            pos_gt_xy = pos_gt_kpts_reshaped[..., :2].to(kpt_pred.dtype)  # (num_pos, K, 2)

            # 写 targets：flatten -> (num_pos, 2K)
            kpt_targets[pos_inds] = pos_gt_xy.reshape(num_pos, -1)

            # 写 weights：xy 都训练 -> 全 1
            pos_w = kpt_pred.new_ones((num_pos, self.num_keypoints, 2))
            kpt_weights[pos_inds] = pos_w.reshape(num_pos, -1)

            # ---- 打印 2：确认 targets 不再全 0（建议 1% 概率）----
        #     if torch.rand(1).item() < 0.01:
        #         print("[_get_target_single] pos_gt_xy min/max:",
        #               float(pos_gt_xy.min()), float(pos_gt_xy.max()))
        #         print("[_get_target_single] kpt_targets[pos] min/max:",
        #               float(kpt_targets[pos_inds].min()),
        #               float(kpt_targets[pos_inds].max()))
        #         print("[_get_target_single] kpt_weights[pos] unique:",
        #               torch.unique(kpt_weights[pos_inds]).detach().cpu().tolist())
        # else:
        #     # ---- 打印 3：如果没有正样本，明确告诉你（建议 5% 概率）----
        #     if torch.rand(1).item() < 0.05:
        #         print("[_get_target_single] WARNING: num_pos == 0, all targets remain zero.")
    ...
    def _get_bboxes_single(self,
        ...
        if self.with_kpt_refine:
            ...
            det_kpts = kpt_pred.reshape(kpt_pred.size(0), -1, 2)  # 2D: (num_query, 14, 2)
