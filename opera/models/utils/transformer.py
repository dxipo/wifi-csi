...
class PetrTransformerDecoder(TransformerLayerSequence):
    ...
    def forward(self,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if kpt_branches is not None:
                tmp = kpt_branches[lid](output)  # (bs, num_query, K*D)

                if reference_points.shape[-1] == self.num_keypoints * 3:
                    new_reference_points = tmp + reference_points
                elif reference_points.shape[-1] == self.num_keypoints * 2:
                    new_reference_points = tmp + reference_points
                else:
                    raise ValueError(
                        f'Unexpected reference_points last dim {reference_points.shape[-1]}, '
                        f'expected {self.num_keypoints * 2} or {self.num_keypoints * 3}'
                    )
...
class PETRTransformer(Transformer):
    ...
    def forward_refine(self,
        ...
        assert reference_points_pose.size(1) % 2 == 0
